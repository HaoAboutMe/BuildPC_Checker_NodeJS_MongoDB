module.exports = (components, errors, warnings, ramQuantity = 1) => {
  ...
  const recommendedPsuWattage = Math.ceil((totalTdp * 1.7) / 50) * 50; // Round up to nearest 50

  if (psu) {
    if (psu.wattage < recommendedPsuWattage) {
      errors.push(
        `Công suất Nguồn (${psu.wattage}W) thấp hơn mức đề xuất (${recommendedPsuWattage}W) cho cấu hình này`,
      );
    }
    ...
    if (vga && vga.powerConnector) {
      ...
      const psuConnectors =
        psu.pcieConnectors?.map((c) => c._id?.toString()) || [];

      if (!psuConnectors.includes(vgaConnectorId)) {
        warnings.push(
          `Nguồn có thể không có đầu cắm VGA tương ứng (${vga.powerConnector.name}). Vui lòng kiểm tra kỹ.`,
        );
      }
    }
  }
